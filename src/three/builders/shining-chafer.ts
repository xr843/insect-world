 ...
 *   金属结构色——鞘翅 elytra('#2f9648', 0.5, {iridescent:true})：铜绿底 +
 *   强虹彩（薄膜干涉随视角在绿-金-紫间流转），与吉丁定标的同类效果；
 *   iridescent 开启时 kit 自动把清漆压到 0.35，铁律不越界。前胸背板/头/
 *   唇基走同色系的高金属 chitin（clearcoat 0.4 < 0.55），腹面与足是铜褐色
 *   （真实铜绿丽金龟腹面即古铜褐）。⚠️ 底色亮度是这只虫的命：首版用了
 *   #0f3a19（近黑），生产目检整只读成「黑铬银」——金属材质的环境反射色
 *   就是底色本身，底色近黑则反射只剩清漆的白光，招牌「铜绿」直接消失。
 *   这正是第 5 轮榆蓝叶甲「金属蓝绿压成近黑」的原样重演；对齐它返工后的
 *   定版亮度（#1c8a8f，V≈0.56）取 #2f9648。金属虫「压深一档」压的是
 *   彩度饱和的中亮档，不是往黑里压。
...
export function buildShiningChafer(): InsectModel {
  ...
  const shellMat = elytra('#2f9648', 0.5, { iridescent: true })
  // 前胸/头/唇基：同色系高金属，微偏暖（真实个体前胸常比鞘翅更泛铜光）
  const noteumMat = chitin({ color: '#1e7a33', gloss: 0.72, metal: 0.5, clearcoat: 0.4 })
