...
| | |
| --- | --- |
| ![分步讲解](docs/screenshots/06-lesson.jpg) | ![对比条](docs/screenshots/07-compare.jpg) |
| 讲解到哪一步，镜头就移到哪个部位 | 展台底部的内联对比条 |
...
逐个点过参考站的每个控件之后，它的实际行为比表面看起来简单：

| 控件 | 参考站 | 这里 |
| --- | --- | --- |
| 导航 Explore / Systems / Library / Notes | 全无反应，连激活态都不变 | 有激活态；搜索实时过滤 |
| 导航 Lessons | 开 "Guided discovery" 弹窗 | 开分步讲解弹窗 |
| Cross-section 剖切 | 生效，裁剪平面切开模型 | 同 |
| Compare 对比 | 生效，展台底部浮出对比条 | 同，另可换对照物种 |
| Isolate / Layers / Zoom / Reset | 无可见变化 | 镜头聚焦部位 / 外骨骼半透明 / 拉近一档 / 复位 |
| 搜索、View all organs | 不过滤、无反应 | 实时过滤下拉；总览按目分组 |
...
| 文件 | 数量 | 管什么 |
| --- | --- | --- |
| `data/__tests__/guides.test.ts` | 387 | 讲解与测验的结构、字数、anchor 逐物种校验 |
| `data/__tests__/insects.test.ts` | 342 | 图鉴数据契约与 anchor 白名单 |
| `components/__tests__/glyph.test.tsx` | 75 | 24 个剪影的结构与坐标越界 |
| `three/__tests__/integration.test.ts` | 75 | 建模层 × 数据层的接缝 |
| `builders/__tests__/kit.test.ts` | 30 | 放样地基与退化输入（重合点、零半径、竖直路径） |
| `builders/__tests__/{flyers,crawlers,hoppers,longbodies,winged2,beetles2,orthoptera2}.test.ts` | 117 | 各物种的几何合法性、形态比例、面数预算、锚点完整性 |
| `builders/__tests__/mirror.test.ts` | 8 | 成对附肢的对称性、`wing` 的 `spread` 语义 |
...
- 顶栏的「分类 / 图鉴库 / 笔记」是布局占位（「探索」与「课程」是实的）。
