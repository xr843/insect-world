...
参考站把**剖切**和**对比**做实了 —— 裁剪平面真的切开模型，展台底部真的浮出
对比条；这两件这里同样做实（对比另可换对照物种）。`Lessons` 也真的会开讲解弹窗。

其余控件多是占位。逐个点过一遍之后，这四类在这里补成了真功能：

| 控件 | 参考站 | 这里 |
| --- | --- | --- |
| 顶栏导航与搜索 | 占位：不切换激活态、不过滤 | 五个入口都有激活态；搜索实时过滤下拉，总览按目分组 |
| 工具条 Isolate / Layers / Zoom / Reset | 点击无可见变化 | 镜头聚焦部位 / 外骨骼半透明 / 拉近一档 / 复位 |
| Quiz 小测 | 三个选项，点击无反馈 | 真能作答：判对错、给解释、统计得分 |
| 讲解弹窗的内容 | 四个变体共用一段静态文字 | 分 3~4 步，每步把 3D 镜头移到讲到的部位 |
...
留下来的是**信息架构**：三栏工作台（图鉴列表 / 3D 展台 / 详情面板）、模型上的彩色
标注点、底部扩展卡片、顶栏五个入口——这一层确实学自参考站，不藏着。
而交互上凡是它做成装饰的，这里都补成了真功能（见上表），这从 v1 起就是本项目的立身之本。
...
[thebuggeddev/anatomy](https://github.com/thebuggeddev/anatomy)）—— 三栏工作台、
模型上的彩色标注点、底部扩展卡片、引导讲解的位置，都来自它。本项目最初（v1）
连配色与版式也是照着它实测取值复刻的；转为公开仓库时，这些借来的数值已全部替换为
自研的两套主题（详见上文「和参考站的对照」一节）。感谢它把这个交互形态做了出来。

学来的不止信息架构，交互手感也有一部分：2026-08-08 对照它的源码之后，这里移植了
四项做法 —— 背面标注点按朝向淡出、自转在拖动/刚松手/说明卡展开时让位、换标本的
进出场节奏、右栏错峰入场（提交记录 `c1f7afa`，相关代码注释里也标着出处）。都是
重新实现的，不是拷贝，但确实是看过它的源码之后才做对的，写在这里比含糊过去好。

需要说明的是，Anatomy Atelier 仓库未附任何 license（即保留所有权利），本项目
不含它的任何代码：借鉴集中在信息架构与上述交互行为，具体实现全部为自写。
