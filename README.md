...
彩色标注点、底部扩展卡片、引导讲解的位置都来自它；2026-08-08 又对照它的源码移植了四项
交互手感（背面标注点按朝向淡出、自转让位、换标本的进出场节奏、右栏错峰入场，见 `c1f7afa`）。
配色与版式在 v1 曾照它复刻，转公开时已全部替换（详见「和参考站的对照」一节）。

该仓库未附任何 license（即保留所有权利）；本项目不含它的任何代码，上述借鉴全部为自写实现。
