...
24 个物种覆盖 11 个目：鞘翅目 7、鳞翅目 3、膜翅目 3、直翅目 3、半翅目 2，以及蜻蜓目、螳螂目、䗛目、双翅目、脉翅目、革翅目各 1。
...
「对比」在展台底部浮出对照条。右栏的「读它的图鉴详解」是分步讲解，每翻一步镜头会自己移到讲到的部位上。
...
单个物种 1.4 万~3.6 万三角面，在浏览器里构建耗时 30~90 毫秒。加一个物种只需再写一个 builder 文件 —— 注册表用 `import.meta.glob` 自动发现，不必改任何现有代码。
