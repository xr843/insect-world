...
双主题视觉：默认**纸感图鉴**（奶油纸底 + 圆顶展台的浅色阅读视图），一键切换自研的
**「博物馆之夜」**暗色——展厅黑做底、黄铜做骨、标本靠聚光灯锥与光池从暗场里托出，
金属鞘翅、薄膜虹彩、萤火虫的尾灯都在深底上起飞。三栏工作台的信息架构（图鉴列表 /
3D 展台 / 详情面板 + 彩色标注点 + 底部扩展卡片）沿自早期参考的
[Anatomy Atelier](https://anatomy-livid.vercel.app/)，视觉皮肤已全部替换。
...
v2 升级为双主题 token 架构（`:root` + `[data-theme]` 两组，见 `src/styles/global.css` 头注）：
默认纸感浅色延续 v1 观感，可切换的暗色是自研的「博物馆之夜」系统（`#131110` 展厅黑、
`#b08d57` 黄铜、Playfair Display + Noto Serif SC、聚光灯池展台，3D 随主题换灯不换材质）。
