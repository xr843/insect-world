...
> **2026-09-23 墙已撤。** 按上线时定的标准：9/8~9/23 共 4681 次访问，
> 投票 8 次、自由留言 0 条（入口打开 59 次：页脚 33 / 浮栏 20），与观察笔记
> （14 次/月）同一量级。`/api/wall`、`/api/wish/vote` 与前端入口已删；
> **纠错**（右栏「这里画得不对？」→ `POST /api/feedback`）照旧，所以下面的
> D1 绑定与 `FEEDBACK_SALT` 都还在用、不能拆。`wishes` / `votes` 两张表与票数
> 原样留在库里。
